...
BaseCache *BuildCacheBank(Config &config, const string &prefix, g_string &name,
  ...
  string arrayType =
      config.get<const char *>(prefix + "array.type", "SetAssoc");
  uint32_t candidates =
  ...
  if (numHashes) {
    if (hashType == "None") {
      ...
      // STL hash function
      size_t seed =
          _Fnv_hash_bytes(prefix.c_str(), prefix.size() + 1, 0xB4AC5B);
      // info("%s -> %lx", prefix.c_str(), seed);
      hf = new H3HashFamily(numHashes, setBits,
                            0xCAC7EAFFA1 +
                                seed /*make randSeed depend on prefix*/);
  ...
  if (arrayType == "SetAssoc") {
    ...
    panic(
        "This should not happen, we already checked for it!"); // unless someone
                                                               // changed
                                                               // arrayStr...
  }
  ...
  uint32_t accLat =
      (isTerminal) ? 0
                   : latency; // terminal caches has no access latency b/c it is
                              // assumed accLat is hidden by the pipeline
...
static void InitSystem(Config &config) {
  ...
  for (const char *grp : cacheGroupNames) {
    ...
    string children =
        config.get<const char *>(prefix + group + ".children", "");
  ...
  if (!zinfo->traceDriven) {
    ...
    for (const char *group : coreGroupNames) {
      if (parentMap.count(group))
        panic("Core group name %s is invalid, a cache group already has that "
              "name",
              group);
      ...
      if (type == "Simple") {
        ...
        zinfo->oooDecode =
            true; // enable uop decoding, this is false by default, must be true
                  // if even one OOO cpu is in the system
      } else if (type == "Null") {
      ...
      if (type != "Null") {
        ...
        for (uint32_t j = 0; j < cores; j++) {
          ...
          if (assignedCaches[icache] >= igroup.size()) {
            panic("%s: icache group %s (%ld caches) is fully used, can't "
                  "connect more cores to it",
          ...
          if (assignedCaches[dcache] >= dgroup.size()) {
            panic("%s: dcache group %s (%ld caches) is fully used, can't "
                  "connect more cores to it",
    ...
    for (const char *grp : cacheGroupNames) {
      if (isTerminal(grp) && assignedCaches[grp] != cMap[grp]->size()) {
        panic("%s: Terminal cache group not fully connected, %ld caches, %d "
              "assigned",
...
static void PostInitStats(bool perProcessDir, Config &config) {
  ...
  if (zinfo->statsPhaseInterval) {
    ...
    if (!prStat)
      panic("No stats match sim.periodicStatsFilter regex (%s)! Set interval "
            "to 0 to avoid periodic stats",
            periodicStatsFilter);
...
void SimInit(const char *configFile, const char *outputDir, uint32_t shmid) {
  ...
  zinfo->outputDir =
      gm_strdup(config.get<const char *>("pin.outputDir", outputDir));
  ...
  if (zinfo->traceDriven) {
    ...
    if (numCores == 0)
      panic("Config must define some core classes in sys.cores; sys.numCores "
            "is deprecated");
    zinfo->numCores = numCores;
    assert(numCores <= MAX_THREADS); // TODO: Is there any reason for this
                                     // limit?
  }
  ...
  if (zinfo->ffReinstrument)
    warn("sim.ffReinstrument = true, switching fast-forwarding on a "
  ...
  if (zinfo->blockingSyscalls) {
    warn("sim.blockingSyscalls = True, will likely deadlock with "
         "multi-threaded apps!");
  }
